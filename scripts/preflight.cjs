// Preflight for the bundled Days of Ruin app: every file the WKWebView shell
...
const required = [
  ...
  'web/img/logo.png', 'web/img/band.jpg', 'web/img/live-crowd.jpg',
  'web/img/cover-aoi.jpg', 'web/img/cover-extinction.jpg', 'web/img/cover-predators.jpg',
...
if (!issues.length) {
  ...
  if (start === -1 || end < start) issues.push('index.html has no inline script.');
    ...
    const tmp = path.join(os.tmpdir(), `days-of-ruin-${process.pid}.js`);
  ...
  if (!project.includes('PRODUCT_BUNDLE_IDENTIFIER: com.sebastianbuergy.daysofruin')) issues.push('project.yml bundle id changed.');
  if (!project.includes('CFBundleDisplayName: Days of Ruin')) issues.push('project.yml display name changed.');
  const html2 = html;
  for (const m of html2.matchAll(/url\((img\/[^)]+)\)/g)) {
    if (!fs.existsSync(path.join(root, 'web', m[1]))) issues.push(`CSS references missing: web/${m[1]}`);
  }
}
